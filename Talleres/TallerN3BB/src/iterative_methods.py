...
# Actualizamos las funciones para guardar las iteraciones
...
def gauss_jacobi(
    *, A: np.ndarray, b: np.ndarray, x: np.ndarray, tol: float, max_iter: int
    ...
    Ahora devuelve los valores de x en cada iteración para graficarlos."""
    if not isinstance(A, np.ndarray):
        A = np.array(A, dtype=float)
    assert A.shape[0] == A.shape[1], "La matriz A debe ser de tamaño n-by-(n)."
...
def gauss_seidel(
    *, A: np.ndarray, b: np.ndarray, x: np.ndarray, tol: float, max_iter: int
    ...
    for k in range(1, max_iter + 1):
        x_new = np.zeros((n, 1))  
        for i in range(n):
            suma = sum([A[i, j] * x_new[j] for j in range(i) if j != i]) + sum([A[i, j] * x[j] for j in range(i, n) if j != i])
            x_new[i] = (b[i] - suma) / A[i, i]
